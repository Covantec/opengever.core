...
class EditMeetingView(ModelEditForm):
    ...
    def inject_initial_data(self):
        ...
        if secretary:
            # XXX - We'll probably want to handle a Member here as well
    ...
    def applyChanges(self, data):
        ...
        # XXX We'll probably want logic here for a Member as well as for a User
        if secretary:
            # We get in the user id from the keyword widget
            ogds_user = ogds_service().fetch_user(secretary)
            if not ogds_user:
                # XXX - Unimplemented
                # We do not have a user for this id, so we create an inactive one
                pass
